...
def main() -> None:
    ...
    with torch.inference_mode():
        ...
        print(
            f"DIJKSTRA optimal parent rate: {dijkstra_metrics.optimal_parent_rate:.4f}"
        )
        ...
        maze_decoded = decode_moves(
            maze_tokens[0].cpu().tolist(), maze_samples[0]["grid"]
        )
        ...
        gsm_tokens = model.gsm8k.generate(questions.to(device), mask.to(device))[
            0
        ].cpu()
        expected = decode_bytes(
            [token for token in targets[0].tolist() if token != -100]
        )
        print(
